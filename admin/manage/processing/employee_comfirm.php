...
if (isset($employee['id_admin'])) {
    if ($employee['lever'] == 1) {
        ...
        if ($type == 1) {
            ...
            $content = "Chào $user <br> Chúng tôi đã xác nhận tài khoản của bạn thành công, chào mừng bạn gia nhập SHOP nè! <br> cảm ơn";
        } elseif ($type == 0) {
            $sql = "DELETE FROM `admin` WHERE `id_admin` = '$id_admin'";
            $title = "Thông báo xác nhận tài khoản thất bại";
            $content = "Chào $user <br> Chúng tôi đã xác nhận tài khoản của bạn rằng không hợp lệ, cảm ơn bạn đã theo giõi SHOP nè! <br> cảm ơn";
            mysqli_query($connection, $sql);
        } elseif ($type == -1) {
            $sql = "UPDATE `admin` SET `status_admin` = '-1' where `id_admin` = '$id_admin'";
            mysqli_query($connection, $sql);
            $title = "Thông báo xác nhận khóa tài khoản";
            $content = "Chào $user <br> Chúng tôi đã nhận thất tài khoản của bạn đã vi phạm tiêu chuẩn công đồng, điều này là không được phép với các thành viên của SHOP nè!<br>Chúng tôi sẽ mở khóa tài khoản sau thời gian cấm kết thúc <br>Báo cáo nhầm lẫn <a href=''>Tại đây</a> <br> cảm ơn";
        } elseif ($type == 2) {
            $sql = "UPDATE `admin` SET `status_admin` = '1' where `id_admin` = '$id_admin'";
            mysqli_query($connection, $sql);
